...
export const EvidenceLocker: React.FC<EvidenceLockerProps> = ({
  ...
  const getSourceIcon = (sourceType: string) => {
    switch (sourceType) {
      case 'CDR': return <PhoneCall className="w-4 h-4 text-sky-400" />;
      case 'FINANCIAL': return <Landmark className="w-4 h-4 text-emerald-400" />;
      case 'FIR': return <FileText className="w-4 h-4 text-blue-400" />;
      case 'INTERROGATION': return <User className="w-4 h-4 text-indigo-400" />;
      case 'PDF_DOCUMENT': return <FileText className="w-4 h-4 text-purple-400" />;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        <div className="flex items-center gap-2 bg-[#080c14] border border-slate-700 rounded px-2.5 py-1.5 flex-1 max-w-md">
        ...
        <div className="flex items-center gap-2 text-xs font-mono">
          ...
          <select
            ...
            className="bg-[#080c14] border border-slate-700 rounded px-2 py-1 text-slate-300 focus:outline-none"
          >
      ...
      {loading ? (
        ...
        <div className="p-12 text-center workstation-card rounded-lg space-y-3">
          ...
          <p className="text-xs text-slate-300 font-mono">No matching evidence found in locker</p>
          <p className="text-[11px] text-slate-500 max-w-md mx-auto font-mono">
        ...
        <div className="space-y-2.5">
          {filteredEvidence.map((item) => (
            <div
              ...
              className="p-4 rounded-lg workstation-card border border-slate-800 hover:border-slate-700 transition-colors cursor-pointer flex flex-col md:flex-row items-start md:items-center justify-between gap-3"
            >
              <div className="space-y-2 flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-[10px] font-mono text-sky-300 bg-sky-950 px-1.5 py-0.5 rounded border border-sky-800 font-bold">
                    {item.evidence_code || 'EVID-SEC-63'}
                  </span>
                  <div className="flex items-center gap-1 px-1.5 py-0.5 rounded bg-[#080c14] border border-slate-800 text-[10px] font-mono text-slate-300">
                ...
                <div className="flex items-baseline gap-2">
                  <span className="font-semibold text-xs text-slate-100 truncate font-mono">{item.file_name}</span>
                ...
                <div className="flex items-center gap-2 bg-[#080c14] px-2.5 py-1 rounded border border-slate-800 text-[10px] font-mono">
                  <Lock className="w-3 h-3 text-sky-400 flex-shrink-0" />
                  <span className="text-slate-500">SHA-256:</span>
                  <span className="text-sky-300 truncate select-all">{item.file_hash_sha256}</span>
                </div>

                {/* Seizure Metadata */}
                <div className="flex flex-wrap items-center gap-3 text-[10px] font-mono text-slate-400">
                  <span>IO: <span className="text-slate-200">{item.seizing_officer || 'Insp Rajesh Sharma'}</span></span>
                  <span>•</span>
                  <span>LOCATION: <span className="text-slate-200">{item.place_of_seizure || 'Cyber Crime PS'}</span></span>
                  <span>•</span>
                  <span>DATE: <span className="text-slate-200">{new Date(item.ingestion_timestamp).toLocaleDateString()}</span></span>
                </div>
              </div>
